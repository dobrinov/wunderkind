...
The product scoping doc (three phases: Foundation → Teachers/Classrooms/Homework → AI/adaptive path) is published as an artifact. Phase 1 shipped the content model v2, gamification core, design system, and platform cleanup; Phase 2 shipped teacher/parent roles, classrooms, homework, daily-minutes mode, and email verification/password reset. Phase 3 (not built) adds the adaptive engine, AI hint ladders, free-text grading, and the nickname-only global weekly leaderboard. Production SMTP delivery is not configured yet — verification/reset emails only log in development.
